<!--
|
| Mostra gráficos da seção Gênero Autor
|
-->
...
if($assunto_filtro == 'Todos') {
    ...
    $result = mysqli_fetch_row( mysqli_query($conn, "SELECT `genero_autor` FROM `mongodata` WHERE `comarca`='$collection'") );
    if($result[0] != NULL) {
    ...
    // PEGA OS DADOS REMOTAMENTE DO MONGO
        ...
        $s = microtime(true);
        $varas = $mongo->genero_autor($collection);
        $e = microtime(true);
        ...
        if(mysqli_num_rows($result = mysqli_query($conn, "SELECT `vara` FROM `mongodata` WHERE `comarca`='$collection'")) == 0) {
            ...
            $sql = "INSERT INTO `mongodata` (`id`, `comarca`, `genero_autor`) VALUES (NULL, '$collection', '$json')";
        ...
        // CASO A TUPLA NO BD EXISTA, ATUALIZA ELA
            ...
            $tipo = "UPDATE";
            #Gera o SQL para inserir no banco
            $sql = "UPDATE `mongodata` SET `genero_autor`='$json' WHERE `comarca`='$collection'";
            //mysqli_query($conn, $sql);
            mysqli_query($conn, $sql)
            or die(mysqli_error($conn));
        }
...
// CASO FILTRO DE ASSUNTO SEJA USADO
    ...
    $tipo = "FILTRO";
    $s = microtime(true);
    $varas = $mongo->genero_autor($collection, $assunto_filtro);
    $e = microtime(true);
